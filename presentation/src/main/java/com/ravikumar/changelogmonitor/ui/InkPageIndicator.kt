...
class InkPageIndicator @JvmOverloads constructor(
  ...
  internal var animHalfDuration: Long = 0
  ...
  internal var selectedDotX: Float = 0.toFloat()
  internal var selectedDotInPosition: Boolean = false
  internal var dotCenterX: FloatArray? = null
  private var joiningFractions: FloatArray? = null
  internal var retreatingJoinX1: Float = 0.toFloat()
  internal var retreatingJoinX2: Float = 0.toFloat()
  private var dotRevealFractions: FloatArray? = null
  private var isViewAttachedToWindow: Boolean = false
  internal var pageChanging: Boolean = false
  ...
  init {
    ...
    animDuration =
      a.getInteger(R.styleable.InkPageIndicator_animationDuration, DEFAULT_ANIM_DURATION)
        .toLong()
    animHalfDuration = animDuration / 2
    val unselectedColour =
    ...
    interpolator = getFastOutSlowInInterpolator(context)
  ...
  internal fun setPageCount(pages: Int) {
    pageCount = pages
    resetState()
    requestLayout()
  }
  ...
  private fun drawUnselected(canvas: Canvas) {
    ...
    for (page in 0 until pageCount) {
      ...
      combinedUnselectedPath.op(
        getUnselectedPath(
          page, dotCenterX!![page], dotCenterX!![nextXIndex],
  ...
  internal fun clearJoiningFractions() {
    Arrays.fill(joiningFractions!!, 0f)
    postInvalidateOnAnimation()
  }

  internal fun setDotRevealFraction(
    dot: Int,
    fraction: Float
  ) {
    dotRevealFractions?.set(dot, fraction)
    postInvalidateOnAnimation()
  }
   ...
   */
  internal abstract inner class PendingStartAnimator(private var predicate: StartPredicate) :
    ValueAnimator() {
   ...
   */
  private inner class PendingRetreatAnimator internal constructor(
    was: Int,
    now: Int,
    steps: Int,
    ...
    init {
      ...
      if (initialX1 != finalX1) { // rightward retreat
        ...
        for (i in 0 until steps) {
          revealAnimations[i] =
            PendingRevealAnimator(was + i, RightwardStartPredicate(dotCenterX!![was + i]))
          dotsToHide[i] = was + i
        }
        ...
        for (i in 0 until steps) {
          revealAnimations[i] =
            PendingRevealAnimator(was - i, LeftwardStartPredicate(dotCenterX!![was - i]))
          dotsToHide[i] = was - i
        }
        addUpdateListener { valueAnimator ->
   ...
   */
  private inner class PendingRevealAnimator internal constructor(
    internal var dot: Int,
    predicate: StartPredicate
  ) : PendingStartAnimator(predicate) {
   ...
   */
  internal abstract inner class StartPredicate(var thresholdValue: Float) {
   ...
   */
  private inner class RightwardStartPredicate internal constructor(thresholdValue: Float) :
    StartPredicate(
      thresholdValue
    ) {
   ...
   */
  private inner class LeftwardStartPredicate internal constructor(thresholdValue: Float) :
    StartPredicate(
      thresholdValue
    ) {
